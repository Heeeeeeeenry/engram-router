...
用户对话 → 原样存入 SQLite
       ↓
  召回: 关键词(FTS5) + 实体图(BFS多跳) + 向量(bge-small-zh 512d)
       ↓
  带证据返回
```

- **三层召回** —— 关键词快速命中 + 实体图多跳联想 + 向量语义兜底
- **零压缩失真** —— 存原文，不摘要
- **证据链可追溯** —— 每条记忆记录来源
- **MCP 标准接口** —— 一行配置接入 Hermes / Claude / Codex / OpenClaw
- **零依赖** —— 核心纯 Python stdlib + SQLite；可选 LLM 特征需 sentence-transformers / faiss
...
```
├── src/engram_router/
│   ├── store.py          # 核心引擎 — SQLite + FTS5 + 向量 + 实体图
