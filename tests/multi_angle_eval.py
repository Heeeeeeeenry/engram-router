...
SCENARIOS.append(TestCase(
    ...
    conversation=[
        ...
        "中午吃了红烧肉，味道还行。",
        "我想明年去日本旅游，看樱花。",
        "张三送了我一把HHKB键盘，因为是生日礼物。",
        "李四说这个键盘不好用。",
    ],
    queries=[
        {"q": "妈妈多大了", "expect": ["55"], "not_expect": [], "desc": "同义词:多大了→年龄"},
        {"q": "李四买特斯拉了吗", "expect": ["比亚迪"], "not_expect": ["特斯拉"], "desc": "否定查询:李四没买特斯拉"},
        {"q": "我为什么心情不好", "expect": ["批评"], "not_expect": [], "desc": "情感推理:心情不好→被批评"},
        {"q": "我之前说的那个计划", "expect": ["日本旅游"], "not_expect": [], "desc": "模糊指代:计划→旅游"},
        {"q": "他送我的东西好用吗", "expect": ["HHKB", "键盘"], "not_expect": ["不好用"], "desc": "人称指代:他→张三,东西→键盘"},
        {"q": "我胖了吗", "expect": ["体重"], "not_expect": [], "desc": "语义相似:胖→体重增加"},
    ],
))
